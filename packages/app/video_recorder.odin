...
video_recorder_desired_frame_count :: proc(elapsed_seconds: f64, fps: u32) -> u64 {
	...
	// The first captured image owns the first output-frame interval. Subsequent
	// images fill every interval reached on the wall clock, repeating an image
	// when rendering is slower than the fixed-rate video timeline.
