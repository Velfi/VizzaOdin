...
Render_Context :: struct {
	...
	video_recorder_readback: engine.Vk_Buffer,
	video_recorder_readback_ready: bool,
	video_recorder_frame_reserved: bool,
	video_recorder_frame_index: int,
}
...
render_backend_init :: proc(backend: ^Render_Backend, ctx: ^engine.Vk_Context) -> bool {
	...
	if !engine.ui_renderer_init(&backend.ui, ctx) {
		return false
	}
...
render_backend_destroy :: proc(backend: ^Render_Backend, ctx: ^engine.Vk_Context) {
	if backend.initialized {
		...
		engine.ui_renderer_destroy(&backend.ui, ctx)
	}
	backend^ = {}
}
...
render_backend_draw_frame :: proc(backend: ^Render_Backend, vk_ctx: ^engine.Vk_Context, sim: ^Gray_Scott_Simulation, preview_gray_scott: ^Gray_Scott_Simulation, particle_life: ^Particle_Life_Simulation, preview_particle_life: ^Particle_Life_Simulation, vectors_gpu: ^Vectors_Gpu_State, preview_vectors_gpu: ^Vectors_Gpu_State, moire_gpu: ^Moire_Gpu_State, preview_moire_gpu: ^Moire_Gpu_State, primordial_gpu: ^Primordial_Gpu_State, preview_primordial_gpu: ^Primordial_Gpu_State, pellets_gpu: ^Pellets_Gpu_State, preview_pellets_gpu: ^Pellets_Gpu_State, flow_gpu: ^Flow_Gpu_State, preview_flow_gpu: ^Flow_Gpu_State, slime_gpu: ^Slime_Gpu_State, voronoi_gpu: ^Voronoi_Gpu_State, preview_slime_gpu: ^Slime_Gpu_State, preview_voronoi_gpu: ^Voronoi_Gpu_State, app_ui: ^App_Ui_State, gui: ^uifw.Gui_Context, dt: f32, app_mode: App_Mode, frame_index: u64, screenshot: ^engine.Screenshot_State, video_recorder: ^Video_Recorder_State = nil) -> bool {
	...
	if !render_graph_execute(&graph, &ctx) {
		...
		if ctx.video_recorder_readback_ready {
			engine.vk_destroy_buffer(vk_ctx, &ctx.video_recorder_readback)
		}
		if ctx.video_recorder_frame_reserved {
			video_recorder_release_frame(video_recorder, ctx.video_recorder_frame_index)
		}
		return false
	}
	...
	if ctx.video_recorder_readback_ready {
		_ = vk.WaitForFences(vk_ctx.device, 1, &frame.state.in_flight, true, 0xffffffffffffffff)
		size := int(vk_ctx.swapchain_extent.width * vk_ctx.swapchain_extent.height * 4)
		pixels := (cast([^]u8)ctx.video_recorder_readback.mapped)[:size]
		_ = video_recorder_submit_reserved_frame(video_recorder, ctx.video_recorder_frame_index, pixels, vk_ctx.swapchain_extent.width, vk_ctx.swapchain_extent.height, vk_ctx.swapchain_format)
		engine.vk_destroy_buffer(vk_ctx, &ctx.video_recorder_readback)
	}
...
render_graph_execute :: proc(graph: ^Render_Graph, ctx: ^Render_Context) -> bool {
	for i in 0 ..< graph.pass_count {
		...
		if pass.execute != nil && !pass.execute(ctx, pass) {
			...
			if pass.kind == .Simulation_Present && video_recorder_is_recording(ctx.video_recorder) && app_ui_mode_allows_video_recording(ctx.app_mode) {
				if video_recorder_reserve_frame(ctx.video_recorder, &ctx.video_recorder_frame_index) {
					ctx.video_recorder_frame_reserved = true
					ctx.video_recorder_readback_ready = vk_cmd_capture_swapchain_to_buffer(ctx.vk_ctx, ctx.frame, &ctx.video_recorder_readback)
					if !ctx.video_recorder_readback_ready {
						video_recorder_release_frame(ctx.video_recorder, ctx.video_recorder_frame_index)
						ctx.video_recorder_frame_reserved = false
						video_recorder_fail(ctx.video_recorder, "Failed to capture video frame")
					}
				}
			}
	}
...
render_pass_gray_scott_compute :: proc(ctx: ^Render_Context, pass: ^Render_Pass_Node) -> bool {
	...
	if ctx.app_mode != .Gray_Scott {
		if ctx.app_mode == .Particle_Life && particle_life_ensure_gpu_runtime(ctx.particle_life, ctx.vk_ctx) {
			particle_life_gpu_step(ctx.particle_life, ctx.vk_ctx, ctx.frame.command_buffer, ctx.dt)
			...
			primordial_gpu_step(ctx.primordial_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &ctx.app_ui.primordial, ctx.dt)
		} else if ctx.app_mode == .Pellets && ctx.app_ui != nil && ctx.pellets_gpu != nil {
			pellets_gpu_step(ctx.pellets_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &ctx.app_ui.pellets, ctx.dt)
		} else if ctx.app_mode == .Flow_Field && ctx.app_ui != nil && ctx.flow_gpu != nil {
			flow_gpu_step(ctx.flow_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &ctx.app_ui.flow_field, ctx.dt)
	...
	gray_scott_gpu_step(ctx.sim, ctx.vk_ctx, ctx.frame.command_buffer, ctx.dt)
	return true
}
...
render_pass_main_menu_preview_step :: proc(ctx: ^Render_Context) {
	...
	if ctx.preview_gray_scott != nil && render_main_menu_preview_mode_visible(ctx, .Gray_Scott) {
		...
		if gray_scott_ensure_gpu_runtime(ctx.preview_gray_scott, ctx.vk_ctx) {
			gray_scott_gpu_step(ctx.preview_gray_scott, ctx.vk_ctx, ctx.frame.command_buffer, ctx.dt)
		}
	...
	if ctx.preview_slime_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Slime_Mold) {
		...
		remaining_sim_step(&preview_slime, ctx.dt)
		slime_width, slime_height := render_main_menu_preview_size_for_mode(ctx, .Slime_Mold)
		slime_gpu_step_preview(ctx.preview_slime_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &preview_slime, slime_width, slime_height)
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
	if ctx.preview_particle_life != nil && render_main_menu_preview_mode_visible(ctx, .Particle_Life) {
		...
		if particle_life_ensure_gpu_runtime(ctx.preview_particle_life, ctx.vk_ctx) {
			particle_life_gpu_step(ctx.preview_particle_life, ctx.vk_ctx, ctx.frame.command_buffer, ctx.dt)
		}
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
	if ctx.preview_flow_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Flow_Field) {
		...
		remaining_sim_step(&preview_flow, ctx.dt)
		flow_width, flow_height := render_main_menu_preview_size_for_mode(ctx, .Flow_Field)
		flow_gpu_step_preview(ctx.preview_flow_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &preview_flow, ctx.dt, flow_width, flow_height)
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
	if ctx.preview_pellets_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Pellets) {
		...
		remaining_sim_step(&preview_pellets, ctx.dt)
		pellets_gpu_step(ctx.preview_pellets_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &preview_pellets, ctx.dt)
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
	if ctx.preview_voronoi_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Voronoi_CA) {
		remaining_sim_step(&ctx.app_ui.voronoi_ca, ctx.dt)
		preview_voronoi := ctx.app_ui.voronoi_ca
		render_main_menu_apply_voronoi_palette(&preview_voronoi.voronoi, palette_name)
		voronoi_gpu_step_size(ctx.preview_voronoi_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &preview_voronoi.voronoi, ctx.dt, preview_voronoi.paused, preview_width, preview_height)
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
	if ctx.preview_moire_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Moire) {
		remaining_sim_step(&ctx.app_ui.moire, ctx.dt)
	...
	if ctx.preview_vectors_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Vectors) {
		remaining_sim_step(&ctx.app_ui.vectors, ctx.dt)
	...
	if ctx.preview_primordial_gpu != nil && render_main_menu_preview_mode_visible(ctx, .Primordial) {
		...
		remaining_sim_step(&preview_primordial, ctx.dt)
		primordial_gpu_step(ctx.preview_primordial_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &preview_primordial, ctx.dt)
		ctx.backend.last_main_menu_preview_warmed_mode_count += 1
	}
}
...
render_pass_main_menu_preview_prepare_slot :: proc(ctx: ^Render_Context, slot: Main_Menu_Preview_Slot) {
	...
	case .Pellets:
		...
		if gpu != nil && gpu.ready {
			...
			pellets_write_static_params_size(gpu, viewport.width * 2, viewport.height * 2, &preview_pellets.pellets)
		}
	case .Voronoi_CA:
		gpu := ctx.preview_voronoi_gpu
		if gpu != nil && gpu.ready && gpu.jfa_image.handle != vk.Image(0) && gpu.jfa_image.layout != .SHADER_READ_ONLY_OPTIMAL {
			voronoi_transition_image(ctx.vk_ctx, cmd, &gpu.jfa_image, .SHADER_READ_ONLY_OPTIMAL)
		}
	case .Moire:
		...
		if gpu != nil && gpu.ready && gpu.state_index < 2 {
			...
			moire_update_texture_descriptor(gpu, ctx.vk_ctx, index)
		}
		if gpu != nil && gpu.ready {
	...
	case .Primordial:
		...
		if gpu != nil && gpu.ready {
			...
			primordial_upload_render_params_for_extent(gpu, &preview_primordial.primordial, viewport.width, viewport.height)
			primordial_upload_background_params(gpu, &preview_primordial.primordial)
		}
	}
}
...
render_main_menu_particle_life_preview_settings :: proc(source: Particle_Life_Settings) -> Particle_Life_Settings {
	...
	preview.analysis_enabled = false
	preview.blob_overlay_enabled = false
...
render_pass_main_menu_preview_present_slot :: proc(ctx: ^Render_Context, slot: Main_Menu_Preview_Slot) {
	...
	case .Pellets:
		if ctx.preview_pellets_gpu != nil && ctx.preview_pellets_gpu.ready {
			pellets_gpu_draw_scene_viewport(ctx.preview_pellets_gpu, ctx.vk_ctx, ctx.frame.command_buffer, &ctx.preview_pellets_gpu.background_pipeline, &ctx.preview_pellets_gpu.render_pipeline, viewport, scissor)
		} else {
			ctx.backend.last_main_menu_preview_skipped_present_count += 1
		}
	case .Voronoi_CA:
...
render_pass_simulation_present :: proc(ctx: ^Render_Context, pass: ^Render_Pass_Node) -> bool {
	...
	if ctx.app_mode == .Main_Menu {
		...
		if draw_ui_in_pass {
			...
			engine.ui_renderer_draw(&ctx.backend.ui, ctx.vk_ctx, ctx.frame.command_buffer, ctx.vk_ctx.swapchain_extent)
	...
	if ctx.app_mode == .Particle_Life {
		if ctx.particle_life.gpu.ready {
			if draw_ui_in_pass {
				particle_life_gpu_present(ctx.particle_life, ctx.vk_ctx, ctx.frame, &ctx.backend.ui)
			} else {
...
render_pass_ui_build :: proc(ctx: ^Render_Context, pass: ^Render_Pass_Node) -> bool {
	_ = pass
	start := time.tick_now()
	ok := engine.ui_renderer_build(&ctx.backend.ui, ctx.vk_ctx, ctx.gui.commands[:])
...
render_pass_ui_overlay :: proc(ctx: ^Render_Context, pass: ^Render_Pass_Node) -> bool {
	_ = pass
	if !engine.ui_renderer_has_overlay_work(&ctx.backend.ui) {
		return true
	}
	force_late_ui_overlay := video_recorder_is_recording(ctx.video_recorder) || render_context_scene_blur_enabled(ctx)
	...
	defer engine.vk_cmd_label_end(ctx.vk_ctx, ctx.frame.command_buffer)
	if !engine.ui_renderer_prepare_backdrop_blur(&ctx.backend.ui, ctx.vk_ctx, ctx.frame) {
